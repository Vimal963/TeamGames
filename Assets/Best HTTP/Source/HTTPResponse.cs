...
namespace BestHTTP
{
    ...
    public class HTTPResponse : IDisposable
    {
        ...
        public const int MinBufferSize = 512 * 1024;
        ...
        public virtual bool Receive(int forceReadRawContentLength = -1, bool readPayloadData = true, bool sendUpgradedEvent = true)
        {
            ...
            if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                VerboseLogging(
                    $"Receive. forceReadRawContentLength: '{forceReadRawContentLength:N0}', readPayloadData: '{readPayloadData:N0}'");
            ...
            catch
            {
                if (baseRequest.Retries >= baseRequest.MaxRetries)
                {
                    HTTPManager.Logger.Warning("HTTPResponse",
                        $"{this.baseRequest.CurrentUri.ToString()} - Failed to read Status Line! Retry is enabled, returning with false.");
                    return false;
                }

                HTTPManager.Logger.Warning("HTTPResponse",
                    $"{this.baseRequest.CurrentUri.ToString()} - Failed to read Status Line! Retry is disabled, re-throwing exception.");
                throw;
            }
            ...
            if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                VerboseLogging($"HTTP Version: '{this.VersionMajor.ToString()}.{this.VersionMinor.ToString()}'");
            ...
            {
                HTTPManager.Logger.Warning("HTTPResponse",
                    $"{this.baseRequest.CurrentUri.ToString()} - Skipping Status Message reading!");
        ...
        protected void ReadChunked(Stream stream)
        {
            ...
            if (HTTPManager.Logger.Level == Logger.Loglevels.All)
                VerboseLogging(
                    $"ReadChunked - hasContentLengthHeader: {hasContentLengthHeader.ToString()}, contentLengthHeader: {contentLengthHeader} realLength: {realLength:N0}");
        ...
        protected void FeedStreamFragment(byte[] buffer, int pos, int length)
        {
            ...
            if (fragmentBufferDataLength + length <= baseRequest.StreamFragmentSize)
            {
                ...
                if (fragmentBufferDataLength == baseRequest.StreamFragmentSize || baseRequest.StreamChunksImmediately)
                {
        ...
        protected void AddStreamedFragment(byte[] buffer, int bufferLength)
        {
            ...
            if (HTTPManager.Logger.Level == Logger.Loglevels.All && buffer != null)
                VerboseLogging(
                    $"AddStreamedFragment buffer length: {bufferLength:N0} UnprocessedFragments: {Interlocked.Read(ref this.UnprocessedFragments):N0}");
...
private void VerboseLogging(string str)
        ...
        protected virtual void Dispose(bool disposing)
        {
            ...
            if (this.baseRequest != null && this.baseRequest.Callback != null)
            {
