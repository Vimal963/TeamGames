            ...
            try
            {
                switch (Type)
                {
                    case AuthenticationTypes.Basic:
                        return string.Concat("Basic ", Convert.ToBase64String(Encoding.UTF8.GetBytes(
                    ...
                    case AuthenticationTypes.Digest:
                    {
                        ...
                        {
                            case "md5":
                                HA1 = $"{credentials.UserName}:{Realm}:{credentials.Password}".CalculateMD5Hash();
                                break;

                            case "md5-sess":
                                if (string.IsNullOrEmpty(this.HA1Sess))
                                    this.HA1Sess =
                                        $"{credentials.UserName}:{Realm}:{credentials.Password}:{Nonce}:{ncvalue}".CalculateMD5Hash();
                                HA1 = this.HA1Sess;
                                break;
