...
namespace BestHTTP
{
    ...
    /// <summary>
    /// Called for every fragment of data downloaded from the server. It's return value indicates whether the plugin free to reuse the dataFragment array.
    /// </summary>
    ...
    /// <param name="dataFragmentLength">Length of the downloaded data. When HTTP/2 is used, it can be larger than the request's StreamFragmentSize.</param>
    ...
    public sealed class HTTPRequest : IEnumerator, IEnumerator<HTTPRequest>
    {
        ...
        /// <summary>
        /// Called for every fragment of data downloaded from the server.
        /// </summary>
        ...
        internal void SendOutTo(Stream stream)
        {
            ...
            try
            {
                ...
                if (HTTPManager.Logger.Level <= Logger.Loglevels.Information)
                    HTTPManager.Logger.Information("HTTPRequest", $"Sending request: '{requestLine}'");
