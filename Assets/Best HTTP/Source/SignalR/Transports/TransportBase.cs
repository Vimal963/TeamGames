            ...
            switch (req.State)
            {
                case HTTPRequestStates.Finished:
                    if (resp.IsSuccess)
                    {
                        ...
                        if (response != "started")
                        {
                            Connection.Error($"Expected 'started' response, but '{response}' found!");
                            return;
                        }
                        ...
                        HTTPManager.Logger.Warning("Transport - " + this.Name,
                            $"Start - request finished Successfully, but the server sent an error. Status Code: {resp.StatusCode}-{resp.Message} Message: {resp.DataAsText} Uri: {req.CurrentUri}");
                    goto default;
        ...
        private void OnAbortRequestFinished(HTTPRequest req, HTTPResponse resp)
        {
            switch (req.State)
            {
                case HTTPRequestStates.Finished:
                    ...
                    {
                        HTTPManager.Logger.Warning("Transport - " + this.Name,
                            $"Abort - Handshake request finished Successfully, but the server sent an error. Status Code: {resp.StatusCode}-{resp.Message} Message: {resp.DataAsText} Uri: {req.CurrentUri}");
