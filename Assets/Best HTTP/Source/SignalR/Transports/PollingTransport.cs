            ...
            switch (req.State)
            {
                ...
                case HTTPRequestStates.Finished:
                    if (resp.IsSuccess)
                    {
                        ...
                        reason =
                            $"Connect - Request Finished Successfully, but the server sent an error. Status Code: {resp.StatusCode}-{resp.Message} Message: {resp.DataAsText}";
                    break;
            ...
            switch (req.State)
            {
                ...
                case HTTPRequestStates.Finished:
                    if (resp.IsSuccess)
                    {
                        ...
                        reason =
                            $"Poll - Request Finished Successfully, but the server sent an error. Status Code: {resp.StatusCode}-{resp.Message} Message: {resp.DataAsText}";
                    break;
