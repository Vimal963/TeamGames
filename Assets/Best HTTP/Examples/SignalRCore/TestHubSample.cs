...
#pragma warning restore
        ...
        public void OnConnectButton()
        {
            ...
            hub.On("Send", (string arg) => AddText($"On '<color=green>Send</color>': '<color=yellow>{arg}</color>'").AddLeftPadding(20));
            hub.On<Person>("Person", (person) => AddText(
                $"On '<color=green>Person</color>': '<color=yellow>{person}</color>'").AddLeftPadding(20));
            hub.On<Person, Person>("TwoPersons", (person1, person2) => AddText(
                $"On '<color=green>TwoPersons</color>': '<color=yellow>{person1}</color>', '<color=yellow>{person2}</color>'").AddLeftPadding(20));
        ...
        private void Hub_OnConnected(HubConnection hub)
        {
            ...
            hub.Invoke<string>("NoParam")
                .OnSuccess(ret => AddText($"'<color=green>NoParam' returned: '<color=yellow>{ret}</color>'").AddLeftPadding(20));
            ...
            hub.Invoke<int>("Add", 10, 20)
                .OnSuccess(result => AddText(
                    $"'<color=green>Add(10, 20)</color>' returned: '<color=yellow>{result}</color>'").AddLeftPadding(20))
                .OnError(error => AddText($"'<color=green>Add(10, 20)</color>' error: '<color=red>{error}</color>'").AddLeftPadding(20));

            hub.Invoke<int?>("NullableTest", 10)
                .OnSuccess(result => AddText(
                    $"'<color=green>NullableTest(10)</color>' returned: '<color=yellow>{result}</color>'").AddLeftPadding(20))
                .OnError(error => AddText(
                    $"'<color=green>NullableTest(10)</color>' error: '<color=red>{error}</color>'").AddLeftPadding(20));
            ...
            hub.Invoke<Person>("GetPerson", "Mr. Smith", 26)
                .OnSuccess(result => AddText(
                    $"'<color=green>GetPerson(\"Mr. Smith\", 26)</color>' returned: '<color=yellow>{result}</color>'").AddLeftPadding(20))
                .OnError(error => AddText(
                    $"'<color=green>GetPerson(\"Mr. Smith\", 26)</color>' error: '<color=red>{error}</color>'").AddLeftPadding(20));
            ...
            hub.Invoke<int>("SingleResultFailure", 10, 20)
                .OnSuccess(result => AddText(
                    $"'<color=green>SingleResultFailure(10, 20)</color>' returned: '<color=yellow>{result}</color>'").AddLeftPadding(20))
                .OnError(error => AddText(
                    $"'<color=green>SingleResultFailure(10, 20)</color>' error: '<color=red>{error}</color>'").AddLeftPadding(20));

            // This call demonstrates IEnumerable<> functions, result will be the yielded numbers.
            hub.Invoke<int[]>("Batched", 10)
                .OnSuccess(result => AddText(
                    $"'<color=green>Batched(10)</color>' returned items: '<color=yellow>{result.Length}</color>'").AddLeftPadding(20))
                .OnError(error => AddText($"'<color=green>Batched(10)</color>' error: '<color=red>{error}</color>'").AddLeftPadding(20));
            ...
            hub.GetDownStreamController<int>("ObservableCounter", 10, 1000)
                .OnItem(result => AddText(
                    $"'<color=green>ObservableCounter(10, 1000)</color>' OnItem: '<color=yellow>{result}</color>'").AddLeftPadding(20))
                .OnSuccess(result => AddText("'<color=green>ObservableCounter(10, 1000)</color>' OnSuccess.").AddLeftPadding(20))
                .OnError(error => AddText(
                    $"'<color=green>ObservableCounter(10, 1000)</color>' error: '<color=red>{error}</color>'").AddLeftPadding(20));

            // A stream request can be cancelled any time.
            var controller = hub.GetDownStreamController<int>("ChannelCounter", 10, 1000);

            controller.OnItem(result => AddText(
                    $"'<color=green>ChannelCounter(10, 1000)</color>' OnItem: '<color=yellow>{result}</color>'").AddLeftPadding(20))
                      .OnSuccess(result => AddText("'<color=green>ChannelCounter(10, 1000)</color>' OnSuccess.").AddLeftPadding(20))
                      .OnError(error => AddText(
                          $"'<color=green>ChannelCounter(10, 1000)</color>' error: '<color=red>{error}</color>'").AddLeftPadding(20));
            ...
            hub.GetDownStreamController<Person>("GetRandomPersons", 20, 2000)
                .OnItem(result => AddText(
                    $"'<color=green>GetRandomPersons(20, 1000)</color>' OnItem: '<color=yellow>{result}</color>'").AddLeftPadding(20))
        ...
        private sealed class Person
        {
            ...
            {
                return $"[Person Name: '{this.Name}', Age: '<color=yellow>{this.Age.ToString()}</color>']";
            }
        }
    }
}
